...
using Ink = WorstHotel.HotelInk;
using Family = WorstHotel.HotelSurfaceFamily;
...
namespace WorstHotel
{
    ...
    public sealed class HotelWorld : MonoBehaviour
    {
        ...
        public void Build()
        {
            ...
            // Switch the shared material, not mesh coordinates or object transforms. World projection
            // survives static combining; every surface outside this subtree retains attached UV0.
            foreach (MeshRenderer renderer in architecture.GetComponentsInChildren<MeshRenderer>(true))
                if (renderer.GetComponent<TextMesh>() == null)
                    renderer.sharedMaterial = HotelTextureLibrary.WithMapping(renderer.sharedMaterial, HotelMappingMode.WorldTriplanar);
                    ...
                    if (visual.condition != item.condition)
                    {
                        ...
                        for (int j = 0; j < visual.surfaces.Length; ++j)
                            if (visual.surfaces[j].GetComponent<TextMesh>() == null)
                                visual.surfaces[j].sharedMaterial = textile && item.condition == "dirty"
                                    ? HotelTextureLibrary.WithInk(visual.originalMaterials[j], Ink.Dirty) : visual.originalMaterials[j];
        ...
        void BuildDanger()
        {
            ...
            Rounded(aid, "First aid cabinet", Vector3.zero, new Vector3(.65f, .66f, .24f), Ink.Cream, Family.Metal, true);
            Box(aid, "Medical green panel", new Vector3(0, .09f, -.132f), new Vector3(.31f, .31f, .018f), Ink.Teal, Family.Metal);
            Box(aid, "Medical cross horizontal", new Vector3(0, .09f, -.148f), new Vector3(.23f, .07f, .012f), Ink.White, Family.Metal);
            Box(aid, "Medical cross vertical", new Vector3(0, .09f, -.15f), new Vector3(.07f, .23f, .012f), Ink.White, Family.Metal);
            Cylinder(aid, "First aid pedestal", new Vector3(0, -.72f, .07f), new Vector3(.07f, .76f, .07f), Ink.Metal, Family.Metal);
            ...
            Rounded(alarm, "Alarm backplate", Vector3.zero, new Vector3(.48f, .62f, .13f), Ink.Burgundy, Family.Metal, true);
            Rounded(alarm, "Alarm guarded button", new Vector3(0, -.07f, .105f), new Vector3(.29f, .2f, .11f), Ink.Red, Family.Rubber, true);
            ...
            Cylinder(alarm, "Alarm post", new Vector3(0, -.78f, 0), new Vector3(.075f, .94f, .075f), Ink.Metal, Family.Metal);
            ...
            for (int number = HotelLayout.FirstRoom; number <= HotelLayout.LastRoom; number++)
            {
                ...
                Rounded(control, "Isolation control plate", Vector3.zero, new Vector3(.16f, .55f, .13f), Ink.Metal, Family.Metal, true);
                Transform lever = Rounded(control, "Latched isolation lever", new Vector3(0, -.06f, -.105f), new Vector3(.065f, .21f, .065f), Ink.Brass, Family.Metal).transform;
                Renderer indicator = Ball(control, "Isolation status lamp", new Vector3(0, .19f, -.084f), new Vector3(.065f, .065f, .025f), Ink.Teal, Family.Ceramic).GetComponent<Renderer>();
        ...
        DangerVisual BuildDangerIncident(int index)
        {
            ...
            for (int i = 0; i < segments; i++)
            {
                ...
                GameObject line = Box(v.zone, "Zone boundary " + i, (from + to) * .5f,
                    new Vector3(.018f, .009f, Vector3.Distance(from, to)), Ink.Gold, Family.Ceramic);
            ...
            v.plate = Rounded(source, "Emergency service face", Vector3.zero, new Vector3(.43f, .38f, .16f), Ink.Burgundy, Family.Metal, true).GetComponent<Renderer>();
            ...
            Tube(electric, "Lightning upper", new Vector3(.07f, .15f, -.1f), new Vector3(-.055f, 0, -.1f), .05f, Ink.Gold, Family.Ceramic);
            Tube(electric, "Lightning cross", new Vector3(-.055f, 0, -.1f), new Vector3(.055f, 0, -.1f), .05f, Ink.Gold, Family.Ceramic);
            Tube(electric, "Lightning lower", new Vector3(.055f, 0, -.1f), new Vector3(-.07f, -.12f, -.1f), .05f, Ink.Gold, Family.Ceramic);
            Transform steam = Group(source, "Steam pressure symbol"); v.steam = steam.gameObject;
            for (int i = -1; i <= 1; i++)
            {
                Tube(steam, "Hot pipe rising line", new Vector3(i * .105f, -.09f, -.1f), new Vector3(i * .105f - .025f, .025f, -.1f), .035f, Ink.White, Family.Ceramic);
                Tube(steam, "Hot pipe bend", new Vector3(i * .105f - .025f, .025f, -.1f), new Vector3(i * .105f, .14f, -.1f), .035f, Ink.White, Family.Ceramic);
            }
            Transform fumes = Group(source, "Corrosive fumes symbol"); v.fumes = fumes.gameObject;
            Rounded(fumes, "Corroded bin safety collar", new Vector3(0, -.1f, 0), new Vector3(.58f, .3f, .58f), Ink.Metal, Family.Metal, true);
            Ball(fumes, "Corrosive droplet", new Vector3(-.08f, .04f, -.11f), new Vector3(.085f, .16f, .045f), Ink.Leaf, Family.Water);
            Tube(fumes, "Corroded surface", new Vector3(-.13f, -.075f, -.11f), new Vector3(.14f, -.075f, -.11f), .035f, Ink.White, Family.Metal);
            Ball(fumes, "Fume bubble", new Vector3(.095f, .095f, -.11f), new Vector3(.085f, .085f, .035f), Ink.Leaf, Family.Water);
            ...
            {
                float x = (i - 1) * .17f;
                Tube(arcs, "Arc rising stroke", new Vector3(x, .26f, 0), new Vector3(x - .07f, .48f, 0), .028f, Ink.Gold, Family.Ceramic);
                Tube(arcs, "Arc return stroke", new Vector3(x - .07f, .48f, 0), new Vector3(x + .03f, .68f, 0), .028f, Ink.Foam, Family.Water);
                Ball(vapour, "Steam curl " + i, new Vector3(x, .43f + i * .13f, 0), new Vector3(.15f, .23f, .13f), Ink.Foam, Family.Water);
                Ball(cloud, "Acid wisp " + i, new Vector3(x, .3f + i * .1f, .09f), new Vector3(.2f, .095f, .14f), Ink.Leaf, Family.Water);
            }
        ...
        void ApplyDanger(HotelState state)
        {
            ...
            foreach (var pair in dangerControls)
            {
                ...
                Tint(pair.Value.indicator, isolated ? Ink.Teal : pending ? Ink.Gold : Ink.Green);
            ...
            for (int i = 0; i < dangerVisuals.Length; i++)
            {
                ...
                Tint(v.plate, incident.isolated || resolved ? Ink.Teal : Ink.Burgundy);
        ...
        void BuildShell()
        {
            ...
            Box(p, "Lobby oak floor", new Vector3(0, -.14f, -2.5f), new Vector3(15.8f, .28f, 9), Ink.Oak, Family.Wood, true);
            for (int x = -7; x <= 7; ++x)
                Box(p, "Long floorboard joint", new Vector3(x, .003f, -2.5f), new Vector3(.016f, .005f, 8.9f), Ink.Wood, Family.Wood);
            for (int z = -6; z <= 1; z += 2)
                for (int x = -7; x <= 7; ++x)
                    Box(p, "Staggered floorboard end", new Vector3(x + .48f, .004f, z + (x % 2) * .65f), new Vector3(.94f, .006f, .014f), Ink.Wood, Family.Wood);
            Box(p, "Hall floor", new Vector3(0, -.12f, 12.5f), new Vector3(3, .24f, 21), Ink.Oak, Family.Wood, true);
            Rug(p, new Vector3(0, .02f, 12.45f), new Vector2(2.1f, 20.6f));
            for (int z = 3; z < 23; z += 2)
            {
                GameObject diamond = Box(p, "Runner diamond", new Vector3(0, .039f, z), new Vector3(.22f, .008f, .22f), Ink.Brass, Family.Carpet);
            ...
            Wall(p, new Vector3(4.95f, 0, -7), new Vector3(5.9f, 3.6f, .2f));
            Box(p, "Entrance lintel", new Vector3(0, 3.17f, -7), new Vector3(4, .86f, .24f), Ink.Wood, Family.Wood, true);
            // An open porch is part of the navigable floor, with a physical boundary beyond it.
            Box(p, "Porch", new Vector3(0, -.16f, -8.25f), new Vector3(8, .32f, 2.5f), Ink.Ceramic, Family.Ceramic, true);
            Box(p, "Porch back rail", new Vector3(0, .5f, -9.55f), new Vector3(8.1f, 1, .16f), Ink.Wood, Family.Wood, true);
            Box(p, "Porch left rail", new Vector3(-4.05f, .5f, -8.25f), new Vector3(.16f, 1, 2.6f), Ink.Wood, Family.Wood, true);
            Box(p, "Porch right rail", new Vector3(4.05f, .5f, -8.25f), new Vector3(.16f, 1, 2.6f), Ink.Wood, Family.Wood, true);
            Box(p, "Lobby ceiling", new Vector3(0, 3.65f, -2.5f), new Vector3(15.8f, .18f, 9), Ink.Cream, Family.Plaster, true);
            Box(p, "Hall ceiling", new Vector3(0, 3.65f, 12.5f), new Vector3(3, .18f, 21), Ink.Cream, Family.Plaster, true);
            for (int sign = -1; sign <= 1; sign += 2)
            {
                ...
                foreach (float z in new[] { 5.5f, 12.5f, 19.5f })
                    Box(p, "Portal lintel", new Vector3(sign * 1.5f, 3.12f, z), new Vector3(.24f, .96f, 1.5f), Ink.Plaster, Family.Plaster, true);
        ...
        void BuildLobby()
        {
            ...
            Rounded(desk, "Reception body", new Vector3(0, -.52f, 0), new Vector3(2.4f, .92f, .74f), Ink.Wood, Family.Wood, true);
            Rounded(desk, "Worn oak counter", new Vector3(0, .025f, 0), new Vector3(2.6f, .16f, .88f), Ink.Oak, Family.Wood, true);
            for (int i = -1; i <= 1; ++i)
            {
                Box(desk, "Burgundy recessed panel", new Vector3(i * .73f, -.47f, -.378f), new Vector3(.64f, .61f, .03f), Ink.Burgundy, Family.Wood);
                Box(desk, "Panel gilt line", new Vector3(i * .73f, -.19f, -.399f), new Vector3(.49f, .017f, .018f), Ink.Brass, Family.Metal);
            }
            Text(desk, "Г Р А Н Д", new Vector3(0, -.41f, -.407f), .12f, Ink.Cream);
            Bell(desk, new Vector3(.8f, .14f, -.14f));
            Rounded(desk, "Guest register", new Vector3(-.55f, .15f, -.12f), new Vector3(.55f, .07f, .36f), Ink.Burgundy, Family.Cloth);
            Box(desk, "Register pages", new Vector3(-.55f, .195f, -.12f), new Vector3(.49f, .018f, .32f), Ink.Paper, Family.Paper);
            Tube(desk, "Pen", new Vector3(-.7f, .215f, -.22f), new Vector3(-.43f, .215f, -.05f), .023f, Ink.Brass, Family.Metal);
            Target(desk.gameObject, "desk", "Ресепшен — смена и гости");
            Box(p, "Hanging hotel sign", new Vector3(0, 2.95f, 1.62f), new Vector3(2.8f, .68f, .17f), Ink.Burgundy, Family.Wood);
            for (int s = -1; s <= 1; s += 2)
                Tube(p, "Sign chain", new Vector3(s * 1.1f, 3.3f, 1.62f), new Vector3(s * 1.1f, 3.57f, 1.62f), .035f, Ink.Brass, Family.Metal);
            ...
            Box(p, "Desk tent card", new Vector3(0, 1.27f, -.92f), new Vector3(.94f, .26f, .035f), Ink.Paper, Family.Paper);
            ...
            Rounded(board, "Board timber frame", Vector3.zero, new Vector3(2.15f, 1.62f, .16f), Ink.Wood, Family.Wood, true);
            Box(board, "Board cork", new Vector3(0, 0, -.096f), new Vector3(1.99f, 1.46f, .045f), Ink.Oak, Family.Wood);
            for (int s = -1; s <= 1; s += 2)
                Box(board, "Board stand", new Vector3(s * .86f, -.86f, .08f), new Vector3(.1f, 1.3f, .35f), Ink.Wood, Family.Wood, true);
            Text(board, "ДЕЛА ОТЕЛЯ", new Vector3(0, .55f, -.133f), .17f, Ink.Cream);
            string[] notes = { "ГОСТИ\nЗаселить\nПомочь\nПроводить", "НОМЕРА\nПостель\nПолотенца\nПорядок", "ПОМНИТЬ\nУлыбаться\nДаже если\nснова течёт" };
            for (int i = 0; i < 3; ++i)
            {
                Box(board, "Pinned note", new Vector3((i - 1) * .62f, -.11f, -.134f), new Vector3(.54f, .77f, .012f), i == 2 ? Ink.Linen : Ink.Paper, Family.Paper);
                Text(board, notes[i], new Vector3((i - 1) * .62f, -.08f, -.15f), .075f, Ink.Dark);
                Ball(board, "Drawing pin", new Vector3((i - 1) * .62f, .21f, -.158f), new Vector3(.065f, .065f, .035f), Ink.Burgundy, Family.Metal);
            }
            ...
            Rounded(coffee, "Tabletop", new Vector3(0, .56f, 0), new Vector3(1.3f, .14f, .75f), Ink.Oak, Family.Wood, true);
            for (int s = -1; s <= 1; s += 2)
                Tube(coffee, "Splayed table leg", new Vector3(s * .49f, .52f, 0), new Vector3(s * .58f, .05f, 0), .11f, Ink.Wood, Family.Wood);
            Cup(coffee, new Vector3(.26f, .66f, 0));
            Box(coffee, "Old magazine", new Vector3(-.27f, .64f, -.02f), new Vector3(.37f, .025f, .25f), Ink.Teal, Family.Paper);
        ...
        void BuildStorage()
        {
            ...
            Box(p, "Storage lintel", new Vector3(-2.1f, 3.1f, -3.6f), new Vector3(.2f, 1, 2), Ink.Plaster, Family.Plaster, true);
            ...
            foreach (string id in new[] { "linen", "towels" })
            {
                ...
                Box(rack, "Rack backing", new Vector3(-.5f, .1f, 0), new Vector3(.08f, 2.1f, 1.14f), Ink.Wood, Family.Wood, true);
                for (int j = -1; j <= 1; ++j)
                    Box(rack, "Oak shelf", new Vector3(0, j * .63f - .18f, 0), new Vector3(1.1f, .09f, 1.14f), Ink.Oak, Family.Wood, true);
                for (int s = -1; s <= 1; s += 2)
                    Box(rack, "Rack upright", new Vector3(.45f, .05f, s * .54f), new Vector3(.075f, 2.1f, .075f), Ink.Brass, Family.Metal, true);
                ...
                Box(rack, "Stock label backing", new Vector3(.559f, .28f, 0), new Vector3(.018f, .2f, .69f), Ink.Paper, Family.Paper);
                for (int j = 0; j < 6; ++j)
                {
            ...
            Rounded(hamper, "Wicker body", new Vector3(0, -.04f, 0), new Vector3(.8f, .83f, .78f), Ink.Oak, Family.Wood, true);
            for (int i = 0; i < 7; ++i)
                Box(hamper, "Wicker weave", new Vector3(0, -.37f + i * .115f, -.401f), new Vector3(.73f, .025f, .018f), Ink.Wood, Family.Wood);
            Rounded(hamper, "Hamper dark opening", new Vector3(0, .39f, 0), new Vector3(.67f, .035f, .64f), Ink.Dark, Family.Wood);
            ...
            Cylinder(bin, "Bin body", new Vector3(0, -.05f, 0), new Vector3(.75f, .89f, .75f), Ink.Teal, Family.Metal, true);
            Cylinder(bin, "Bin lid", new Vector3(0, .42f, 0), new Vector3(.83f, .1f, .83f), Ink.Dark, Family.Metal);
            ...
            Box(p, "Tool parking mat", new Vector3(-3.2f, .008f, -.7f), new Vector3(1.05f, .015f, 1.12f), Ink.Dark, Family.Rubber);
        ...
        void BuildServices()
        {
            ...
            Box(utility, "Utility station backboard", new Vector3(-6.8f, 1.15f, -5.66f), new Vector3(1.97f, 1.8f, .12f), Ink.Wood, Family.Wood, true);
            ...
            Tube(water, "Water main pipe", new Vector3(0, -.68f, .1f), new Vector3(0, .43f, .1f), .095f, Ink.Metal, Family.Metal);
            GameObject wheel = Cylinder(water, "Water shutoff wheel", new Vector3(0, 0, -.06f), new Vector3(.38f, .075f, .38f), Ink.Teal, Family.Metal, true);
            wheel.transform.localRotation = Quaternion.Euler(90, 0, 0);
            for (int i = -1; i <= 1; i += 2)
                Tube(water, "Valve wheel spokes", new Vector3(i * -.16f, -.16f, -.11f), new Vector3(i * .16f, .16f, -.11f), .035f, Ink.Brass, Family.Metal);
            ...
            Rounded(power, "Fuse cabinet", Vector3.zero, new Vector3(.57f, .82f, .23f), Ink.Metal, Family.Metal, true);
            for (int i = 0; i < 3; ++i)
                Rounded(power, "Breaker switch", new Vector3((i - 1) * .16f, .04f, -.15f), new Vector3(.085f, .23f, .055f), Ink.Dark, Family.Rubber);
            ...
            Rounded(coffee, "Coffee cabinet", new Vector3(0, -.55f, 0), new Vector3(1.2f, .86f, .67f), Ink.Teal, Family.Wood, true);
            Rounded(coffee, "Coffee counter", new Vector3(0, -.06f, 0), new Vector3(1.3f, .12f, .79f), Ink.Oak, Family.Wood, true);
            Transform basic = Group(coffee, "Manual coffee service"); basicCoffee = basic.gameObject;
            Cylinder(basic, "Insulated coffee pot", new Vector3(-.25f, .23f, .02f), new Vector3(.32f, .43f, .32f), Ink.Metal, Family.Metal, true);
            Cylinder(basic, "Coffee pot lid", new Vector3(-.25f, .46f, .02f), new Vector3(.36f, .045f, .36f), Ink.Dark, Family.Rubber);
            Handle(basic, new Vector3(-.48f, .14f, .03f), .12f, .19f, Ink.Dark, Family.Rubber);
            Tube(basic, "Coffee pot spout", new Vector3(-.14f, .38f, .02f), new Vector3(-.04f, .43f, -.1f), .065f, Ink.Metal, Family.Metal);
            Transform machine = Group(coffee, "Upgraded coffee machine"); upgradedCoffee = machine.gameObject;
            Rounded(machine, "Espresso machine", new Vector3(-.16f, .29f, .05f), new Vector3(.63f, .55f, .45f), Ink.Burgundy, Family.Metal, true);
            Box(machine, "Coffee machine front", new Vector3(-.16f, .27f, -.193f), new Vector3(.52f, .36f, .03f), Ink.Metal, Family.Metal);
            Tube(machine, "Coffee nozzle", new Vector3(-.16f, .25f, -.23f), new Vector3(-.16f, .14f, -.23f), .045f, Ink.Brass, Family.Metal);
            Ball(machine, "Coffee ready light", new Vector3(.02f, .37f, -.225f), Vector3.one * .047f, Ink.Green, Family.Ceramic);
            ...
            foreach(float side in new[]{-1f,1f})
                Box(coffee, "Coffee sign support", new Vector3(side*.47f,.37f,.26f),new Vector3(.027f,.86f,.027f),Ink.Brass, Family.Metal);
            Box(coffee, "Coffee service sign",new Vector3(0,.8f,.26f),new Vector3(1.1f,.24f,.045f),Ink.Cream, Family.Paper);
        ...
        static void ApplyUtility(EquipmentVisual visual, bool fault, float wear, string label)
        {
            ...
            if (visual.code != code)
            {
                ...
                Tint(visual.indicator, fault ? Ink.Red : Ink.Green); visual.code = code;
            }
        ...
        void BuildRoom(int n)
        {
            ...
            Box(p, "Bedroom floor", c + new Vector3(0, -.12f, 0), new Vector3(6.25f, .24f, 6.85f), Ink.Oak, Family.Wood, true);
            Box(p, "Bedroom ceiling", c + new Vector3(0, 3.65f, 0), new Vector3(6.25f, .18f, 6.85f), Ink.Cream, Family.Plaster, true);
            for (int i = 0; i < 8; ++i)
                Box(p, "Bedroom floorboard seam", c + new Vector3(s * (-2.7f + i * .76f), .002f, 0), new Vector3(.014f, .004f, 6.7f), Ink.Wood, Family.Wood);
            Rug(p, c + new Vector3(s * .35f, .014f, 1.16f), new Vector2(3.5f, 2.2f));
            for (int x = 0; x < 5; ++x)
                for (int z = 0; z < 3; ++z)
                    Box(p, "Washroom tile", c + new Vector3(s * (-1.8f + x * .91f), .01f, -2.98f + z * .64f),
                        new Vector3(.88f, .016f, .61f), (x + z) % 2 == 0 ? Ink.Ceramic : Ink.Cream, Family.Ceramic);
            ...
            Rounded(bed, "Timber bed frame", new Vector3(0, -.34f, 0), new Vector3(2.65f, .28f, 1.47f), Ink.Wood, Family.Wood, true);
            Rounded(bed, "Mattress ticking", new Vector3(0, -.1f, 0), new Vector3(2.5f, .29f, 1.42f), Ink.Linen, Family.Cloth, true);
            Rounded(bed, "Scalloped headboard", new Vector3(s * 1.24f, .02f, 0), new Vector3(.16f, 1.2f, 1.54f), Ink.Wood, Family.Wood, true);
            for (int j = -1; j <= 1; ++j)
            {
                Renderer fabric = Rounded(bed, "Upholstered headboard inset", new Vector3(s * 1.135f, .2f, j * .44f), new Vector3(.052f, .57f, .34f), Ink.Burgundy, Family.Cloth).GetComponent<Renderer>();
                if (j == 0) visual.bedFabric = fabric;
            }
            for (int x = -1; x <= 1; x += 2)
                for (int z = -1; z <= 1; z += 2)
                    Cylinder(bed, "Turned bed foot", new Vector3(x * 1.08f, -.55f, z * .58f), new Vector3(.13f, .3f, .13f), Ink.Wood, Family.Wood);
            Target(bedTarget.gameObject, "bed_" + n, "Кровать — снять / застелить бельё");
            Transform clean = Group(bed, "Clean bedding");
            Rounded(clean, "Clean duvet", new Vector3(-s * .17f, .075f, 0), new Vector3(2.1f, .19f, 1.4f), Ink.White, Family.Cloth);
            visual.linenBand = Rounded(clean, "Burgundy runner", new Vector3(-s * .78f, .185f, 0), new Vector3(.43f, .045f, 1.43f), Ink.Burgundy, Family.Cloth).GetComponent<Renderer>();
            for (int z = -1; z <= 1; z += 2)
                Rounded(clean, "Plump pillow", new Vector3(s * .9f, .125f, z * .35f), new Vector3(.5f, .24f, .57f), Ink.White, Family.Cloth);
            visual.clean = clean.gameObject;
            Transform dirty = Group(bed, "Dirty bedding");
            Rounded(dirty, "Crumpled sheet", new Vector3(-s * .21f, .085f, .01f), new Vector3(1.99f, .18f, 1.36f), Ink.Dirty, Family.Cloth);
            for (int j = 0; j < 5; ++j)
            {
                GameObject fold = Rounded(dirty, "Rumpled blanket fold", new Vector3(-.77f + j * .35f, .2f, .05f), new Vector3(.33f, .18f, 1.12f), j % 2 == 0 ? Ink.Linen : Ink.Dirty, Family.Cloth);
                fold.transform.localRotation = Quaternion.Euler(0, (j % 2 == 0 ? -12 : 9), 8);
            }
            Ball(dirty, "Tea stain", new Vector3(.25f, .304f, -.35f), new Vector3(.36f, .014f, .26f), Ink.Wood, Family.Cloth);
            Rounded(dirty, "Forgotten pillow", new Vector3(s * .83f, .15f, .35f), new Vector3(.55f, .24f, .55f), Ink.Linen, Family.Cloth).transform.localRotation = Quaternion.Euler(0, 24, 0);
            visual.dirty = dirty.gameObject;
            Transform upgraded = Group(bed, "Bed upgrade / brass finials");
            for (int z = -1; z <= 1; z += 2)
            {
                Cylinder(upgraded, "Upgrade headboard post", new Vector3(s * 1.23f, .22f, z * .77f), new Vector3(.08f, 1.27f, .08f), Ink.Brass, Family.Metal);
                Ball(upgraded, "Upgrade finial", new Vector3(s * 1.23f, .9f, z * .77f), Vector3.one * .17f, Ink.Brass, Family.Metal);
            }
            visual.upgraded = upgraded.gameObject;
            Transform luxury = Group(clean, "Premium linen embroidery");
            for (int j = -1; j <= 1; j += 2)
                Box(luxury, "Linen embroidered edge", new Vector3(-s * .12f, .18f, j * .57f), new Vector3(1.86f, .012f, .023f), Ink.Brass, Family.Cloth);
            ...
            Rounded(sink, "Washstand cabinet", new Vector3(0, -.55f, 0), new Vector3(1.2f, .79f, .68f), Ink.Teal, Family.Wood, true);
            Rounded(sink, "Ceramic basin rim", new Vector3(0, -.02f, 0), new Vector3(1.32f, .18f, .79f), Ink.Ceramic, Family.Ceramic, true);
            Ball(sink, "Basin hollow", new Vector3(0, .07f, -.02f), new Vector3(.88f, .032f, .5f), Ink.Metal, Family.Ceramic);
            Cylinder(sink, "Drain", new Vector3(0, .095f, -.01f), new Vector3(.095f, .011f, .095f), Ink.Dark, Family.Metal);
            Tube(sink, "Tap stem", new Vector3(0, .05f, -.24f), new Vector3(0, .34f, -.24f), .075f, Ink.Brass, Family.Metal);
            Tube(sink, "Tap spout", new Vector3(0, .34f, -.24f), new Vector3(0, .34f, .06f), .075f, Ink.Brass, Family.Metal);
            Tube(sink, "Tap handle", new Vector3(-.17f, .2f, -.24f), new Vector3(.17f, .2f, -.24f), .035f, Ink.Brass, Family.Metal);
            for (int j = -1; j <= 1; j += 2)
                Ball(sink, "Cabinet knob", new Vector3(j * .14f, -.43f, .353f), Vector3.one * .085f, Ink.Brass, Family.Metal);
            ...
            Rounded(p, "Mirror brass frame", mirrorAt, new Vector3(1.25f, 1.24f, .09f), Ink.Brass, Family.Metal);
            Rounded(p, "Mirror stylized blue reflection", mirrorAt + new Vector3(0, 0, .057f), new Vector3(1.08f, 1.07f, .035f), Ink.Blue, Family.Ceramic);
            Tube(p, "Mirror glint", mirrorAt + new Vector3(-.33f, .16f, .081f), mirrorAt + new Vector3(.2f, .44f, .081f), .025f, Ink.Foam, Family.Water);
            Transform leak = Group(dynamicRoom, "Visible leak", sinkAt);
            Tube(leak, "Split drain pipe", new Vector3(.22f, -.22f, .35f), new Vector3(.32f, -.52f, .38f), .065f, Ink.Water, Family.Metal);
            HotelLeak drip = leak.gameObject.AddComponent<HotelLeak>();
            drip.drops = new Transform[5];
            for (int j = 0; j < drip.drops.Length; ++j)
                drip.drops[j] = Ball(leak, "Falling droplet", new Vector3(.3f, -.2f - j * .13f, .41f), new Vector3(.045f, .085f, .045f), Ink.Water, Family.Water).transform;
            ...
            for (int j = 0; j < 5; ++j)
            {
                ...
                Ball(water, "Uneven water lobe", new Vector3(Mathf.Sin(angle) * .34f, 0, Mathf.Cos(angle) * .25f), new Vector3(1.29f, .045f, .94f), Ink.Water, Family.Water);
            }
            Tube(water, "Puddle reflection", new Vector3(-.43f, .026f, -.09f), new Vector3(.16f, .026f, -.21f), .025f, Ink.Foam, Family.Water);
            Tube(water, "Small reflection", new Vector3(.14f, .026f, .26f), new Vector3(.44f, .026f, .2f), .017f, Ink.Foam, Family.Water);
            ...
            for (int j = -1; j <= 1; j += 2)
            {
                Tube(stand, "Towel rail upright", new Vector3(j * .36f, -.97f, 0), new Vector3(j * .36f, .12f, 0), .045f, Ink.Brass, Family.Metal);
                Box(stand, "Rail foot", new Vector3(j * .36f, -1.05f, 0), new Vector3(.18f, .08f, .45f), Ink.Brass, Family.Metal, true);
            }
            Tube(stand, "Towel top rail", new Vector3(-.38f, .11f, 0), new Vector3(.38f, .11f, 0), .055f, Ink.Brass, Family.Metal);
            ...
            Rounded(towel, "Hanging folded towel", new Vector3(0, -.17f, .03f), new Vector3(.52f, .56f, .1f), Ink.White, Family.Cloth);
            Box(towel, "Towel woven stripe", new Vector3(0, -.36f, .089f), new Vector3(.49f, .047f, .016f), Ink.Teal, Family.Cloth);
            visual.towel = towel.gameObject;

            Transform trash = Group(p, "Room bin", HotelLayout.RoomTarget("trash", n));
            Cylinder(trash, "Wastebasket", new Vector3(0, -.025f, 0), new Vector3(.44f, .51f, .44f), Ink.Wood, Family.Wood, true);
            Cylinder(trash, "Wastebasket opening", new Vector3(0, .237f, 0), new Vector3(.37f, .018f, .37f), Ink.Dark, Family.Wood);
            ...
            for (int j = 0; j < 5; ++j)
                Ball(rubbish, "Crumpled paper", new Vector3(Mathf.Sin(j * 2f) * .14f, .25f + j * .023f, Mathf.Cos(j * 2f) * .13f), Vector3.one * .18f, j % 2 == 0 ? Ink.Paper : Ink.Linen, Family.Paper);
            Rounded(rubbish, "Discarded carton", new Vector3(.3f, -.22f, .18f), new Vector3(.17f, .16f, .27f), Ink.Red, Family.Paper).transform.localRotation = Quaternion.Euler(0, 32, 18);
            visual.trash = rubbish.gameObject;

            Transform luggage = Group(p, "Luggage stand", HotelLayout.RoomTarget("bag", n));
            Rounded(luggage, "Luggage stand top", new Vector3(0, -.05f, 0), new Vector3(.92f, .095f, .69f), Ink.Wood, Family.Wood, true);
            for (int j = -1; j <= 1; j += 2)
            {
                Tube(luggage, "Folding luggage leg", new Vector3(-.4f, -.42f, j * .25f), new Vector3(.37f, -.1f, j * .25f), .055f, Ink.Metal, Family.Metal);
                Tube(luggage, "Folding luggage leg", new Vector3(.4f, -.42f, j * .25f), new Vector3(-.37f, -.1f, j * .25f), .055f, Ink.Metal, Family.Metal);
            }
            ...
            Rounded(legacy, "Bedside cabinet", c + new Vector3(s * 2.05f, .37f, 2.46f), new Vector3(1.1f, .74f, .76f), Ink.Wood, Family.Wood, true);
            ...
            Rounded(tv, "TV casing", Vector3.zero, new Vector3(1.13f, .73f, .2f), Ink.Dark, Family.Rubber);
            Rounded(tv, "TV sleepy screen", new Vector3(-.055f, .02f, -.12f), new Vector3(.86f, .52f, .03f), Ink.Teal, Family.Ceramic);
            ...
            for (int j = -1; j <= 1; j += 2)
                Box(door, "Portal jamb outside clear opening", new Vector3(0, 1.31f, j * .805f), new Vector3(.32f, 2.62f, .11f), Ink.Wood, Family.Wood, true);
            Box(door, "Portal crown", new Vector3(0, 2.66f, 0), new Vector3(.35f, .15f, 1.74f), Ink.Oak, Family.Wood, true);
            ...
            Rounded(openLeaf, "Door leaf fixed open", new Vector3(s * .84f, 1.23f, .88f), new Vector3(1.47f, 2.46f, .12f), Ink.Wood, Family.Wood, true);
            Box(openLeaf, "Open leaf inset", new Vector3(s * .84f, 1.31f, .801f), new Vector3(1.15f, 1.83f, .035f), Ink.Oak, Family.Wood);
            Ball(openLeaf, "Door handle", new Vector3(s * 1.39f, 1.06f, .765f), Vector3.one * .085f, Ink.Brass, Family.Metal);
            ...
            Rounded(locked, "Locked room barrier", new Vector3(0, .3f, 0), new Vector3(.2f, 2.6f, 1.5f), Ink.Wood, Family.Wood, true);
            Box(locked, "Locked room brass band", new Vector3(-s * .115f, .3f, 0), new Vector3(.027f, .15f, 1.34f), Ink.Brass, Family.Metal);
            ...
            Rounded(door, "Room number plaque", new Vector3(-s * .165f, 1.74f, 1.04f), new Vector3(.04f, .49f, .61f), Ink.Burgundy, Family.Metal, true);
            ...
            visual.lamp = Ball(living, "Room status lamp", doorAt + new Vector3(-s * .191f, 2.09f, 1.04f), new Vector3(.075f, .1f, .1f), Ink.Green, Family.Ceramic).GetComponent<Renderer>();
        ...
        void BuildRoomMvp(int n, RoomVisual v, Transform conditions)
        {
            ...
            Ball(v.equipment["toilet"].failure.transform, "Blocked toilet contents", new Vector3(0, -.8f, .34f), new Vector3(.3f, .035f, .38f), Ink.Dirty, Family.Water);
            ...
            v.tvBezel = Rounded(tv, "TV case", screenAt, new Vector3(1.1f, .72f, .19f), Ink.Dark, Family.Rubber, true).GetComponent<Renderer>();
            Box(tv, "Ceiling TV bracket", screenAt + new Vector3(0, .55f, .04f), new Vector3(.08f, .47f, .08f), Ink.Metal, Family.Metal, true);
            Renderer screen = Rounded(tv, "Powered TV screen", screenAt + new Vector3(0, 0, -.112f), new Vector3(.91f, .54f, .028f), Ink.Teal, Family.Ceramic).GetComponent<Renderer>();
            ...
            for (int i = 0; i < 4; ++i)
                Box(broadcast, "Broadcast colour bar", new Vector3(-.3f + .2f * i, -.12f, 0), new Vector3(.19f, .13f, .008f), i % 2 == 0 ? Ink.Gold : Ink.Blue, Family.Ceramic);
            television.working = broadcast.gameObject;
            television.surface = screen;
            Transform tvTrim = Group(tv, "Upgraded TV trim", screenAt);
            for (int side = -1; side <= 1; side += 2)
                Box(tvTrim, "TV quality trim", new Vector3(side * .54f, 0, -.106f), new Vector3(.025f, .64f, .024f), Ink.Brass, Family.Metal);
            ...
            Box(lamp, "Lamp wall shelf", new Vector3(s * .25f, -.44f, 0), new Vector3(.57f, .08f, .46f), Ink.Oak, Family.Wood, true);
            ...
            Rounded(coffee, "Coffee table top", new Vector3(0, -.065f, 0), new Vector3(.62f, .1f, .55f), Ink.Oak, Family.Wood, true);
            Cylinder(coffee, "Coffee table column", new Vector3(0, -.44f, 0), new Vector3(.1f, .76f, .1f), Ink.Wood, Family.Wood, true);
            Cylinder(coffee, "Coffee table foot", new Vector3(0, -.79f, 0), new Vector3(.43f, .08f, .43f), Ink.Wood, Family.Wood, true);
            ...
            {
                float a = i * 2.39996f;
                GameObject spot = Ball(clean, "Dry dirt patch " + i, new Vector3(Mathf.Sin(a) * (i < 3 ? .34f : 1.1f), -.034f, Mathf.Cos(a) * .75f), new Vector3(.22f + (i % 3) * .08f, .015f, .18f), Ink.Dirty, Family.Plaster);
            ...
            Rounded(laundry, "Laundry catch tray", new Vector3(0, -.15f, 0), new Vector3(.59f, .055f, .43f), Ink.Wood, Family.Wood, true);
            for (int i = 0; i < 3; ++i)
            {
            ...
            for (int i = 0; i < 3; ++i)
            {
                GameObject paper = Ball(overflow, "Overflow litter " + i, new Vector3(-.3f + i * .26f, -.26f, .34f), new Vector3(.2f, .08f, .2f), Ink.Paper, Family.Paper);
                v.overflow.Add(paper); SetActive(paper, false);
            }
            v.finishWall = Box(p, "Selectable wall finish", c + new Vector3(0, 1.75f, -3.38f), new Vector3(5.95f, 1.28f, .018f), Ink.Plaster, Family.Plaster).GetComponent<Renderer>();
            v.finishRug = Box(p, "Selectable rug inset", c + new Vector3(s * .35f, .043f, 1.16f), new Vector3(2.7f, .008f, 1.7f), Ink.Burgundy, Family.Carpet).GetComponent<Renderer>();
        ...
        static EquipmentVisual BuildEquipmentPanel(Transform parent, string id, GameObject model, Vector3 at, Vector3 offset)
        {
            ...
            Box(panel, "Service label backing", Vector3.zero, new Vector3(.64f, .2f, .025f), Ink.Cream, Family.Metal, true);
            var v = new EquipmentVisual { root = root.gameObject, model = model };
            v.indicator = Ball(panel, "Equipment indicator", new Vector3(-.26f, 0, -.027f), new Vector3(.055f, .055f, .025f), Ink.Green, Family.Ceramic).GetComponent<Renderer>();
            ...
            for (int i = 0; i < 3; ++i)
            {
                GameObject bar = Box(wear, "Wear mark " + i, new Vector3((i - 1) * .15f, 0, 0), new Vector3(.12f, .022f, .022f), Ink.Gold, Family.Metal);
                v.wear.Add(bar); SetActive(bar, false);
            }
            Transform failure = Group(root, "Local fault", offset);
            Tube(failure, "Fault slash", new Vector3(-.12f, .2f, 0), new Vector3(.12f, .39f, 0), .045f, Ink.Red, Family.Metal);
            Tube(failure, "Fault slash", new Vector3(.12f, .2f, 0), new Vector3(-.12f, .39f, 0), .045f, Ink.Red, Family.Metal);
            ...
            v.unavailable = Box(root, "Utility unavailable", offset + new Vector3(0, .29f, 0), new Vector3(.31f, .1f, .035f), Ink.Gold, Family.Metal, true);
            v.missing = Box(root, "Empty equipment slot", offset + new Vector3(0, .29f, 0), new Vector3(.36f, .18f, .035f), Ink.Dark, Family.Metal, true);
        ...
        static bool ApplyEquipment(EquipmentVisual v, MvpEquipmentState equipment, bool utilityFault, string utilityLabel, bool legacy = false)
        {
            ...
            if (v.surface != null) Tint(v.surface, code == 0 ? Ink.Teal : Ink.Dark);
            if (v.code != code)
            {
                v.status.text = code == 3 ? "НЕТ ПРИБОРА" : code == 2 ? "МЕСТНАЯ ПОЛОМКА" : code == 1 ? utilityLabel : "ИСПРАВНО";
                Tint(v.indicator, code == 3 ? Ink.Dark : code == 2 ? Ink.Red : code == 1 ? Ink.Gold : Ink.Green);
                v.code = code;
            }
        ...
        void ApplyRoom(HotelState state, RoomState room, RoomVisual v)
        {
            ...
            Tint(v.lampGlow, lampWorking ? Ink.Glow : Ink.Dark);
            Tint(v.ceilingGlow, lampWorking && !powerFault ? Ink.Glow : Ink.Dark);
            ...
            if (v.capacity != capacity || v.bedQuality != bedQuality || v.tvQuality != tvQuality)
            {
                ...
                Tint(v.bedFabric, bedQuality >= 3 ? Ink.Teal : bedQuality >= 2 ? Ink.Brass : Ink.Burgundy);
                Tint(v.tvBezel, tvQuality >= 2 ? Ink.Metal : Ink.Dark);
            ...
            if (v.finish != finish)
            {
                Tint(v.finishWall, finish == "warm" ? Ink.Cream : finish == "cool" ? Ink.Blue : Ink.Plaster);
                Tint(v.finishRug, finish == "warm" ? Ink.Red : finish == "cool" ? Ink.Teal : Ink.Burgundy);
                v.finish = finish;
            }
            bool linenUpgrade = mvp && state.mvp.betterLinen;
            SetActive(v.luxuryLinen, linenUpgrade); Tint(v.linenBand, linenUpgrade ? Ink.Teal : Ink.Burgundy);
            ...
            if (flag != v.flag)
            {
                ...
                Tint(v.lamp, color); v.status.color = ColorOf(color); v.flag = flag;
            }
        }
        ...
        public static GameObject MakeItem(string kind, string size, string condition)
        {
            ...
            {
                case "plunger":
                    Shape(p, "Plunger rubber cup", cone, new Vector3(0, .115f, 0), new Vector3(.32f, .18f, .32f), Ink.Burgundy, Family.Rubber);
                    Cylinder(p, "Plunger suction rim", new Vector3(0, .03f, 0), new Vector3(.35f, .06f, .35f), Ink.Dark, Family.Rubber);
                    Tube(p, "Plunger wooden handle", new Vector3(0, .18f, 0), new Vector3(0, 1.04f, 0), .045f, Ink.Oak, Family.Wood);
                    Ball(p, "Plunger grip", new Vector3(0, 1.05f, 0), new Vector3(.071f, .16f, .071f), Ink.Wood, Family.Rubber);
                    break;
                case "coffee":
                case "coffeecup":
                    Cylinder(p, "Coffee saucer", new Vector3(0, .016f, 0), new Vector3(.27f, .03f, .27f), Ink.Ceramic, Family.Ceramic);
                    Cup(p, new Vector3(0, .032f, 0));
                    Box(p, "Coffee service napkin", new Vector3(.025f, .01f, .035f), new Vector3(.28f, .012f, .23f), Ink.Paper, Family.Paper);
                    break;
                case "toolbox":
                    Rounded(p, "Steel toolbox", new Vector3(0, .19f, 0), new Vector3(.64f, .34f, .35f), Ink.Red, Family.Metal);
                    Rounded(p, "Toolbox lid", new Vector3(0, .36f, 0), new Vector3(.68f, .12f, .38f), Ink.Burgundy, Family.Metal);
                    Handle(p, new Vector3(0, .43f, 0), .29f, .12f, Ink.Metal, Family.Metal);
                    for (int j = -1; j <= 1; j += 2)
                        Box(p, "Toolbox latch", new Vector3(j * .19f, .31f, -.196f), new Vector3(.063f, .12f, .027f), Ink.Brass, Family.Metal);
                    Box(p, "Toolbox badge", new Vector3(0, .17f, -.184f), new Vector3(.23f, .105f, .017f), Ink.Paper, Family.Paper);
                    Text(p, "РЕМОНТ", new Vector3(0, .17f, -.196f), .045f, Ink.Dark);
                    break;
                case "mop":
                    Rounded(p, "Mop head", new Vector3(0, .055f, 0), new Vector3(.55f, .1f, .24f), Ink.Teal, Family.Rubber);
                    for (int j = 0; j < 11; ++j)
                        Tube(p, "Cotton mop strand", new Vector3(-.25f + j * .05f, .055f, -.12f), new Vector3(-.24f + j * .049f, .027f, .18f + (j % 3) * .025f), .034f, Ink.Linen, Family.Cloth);
                    Tube(p, "Wood mop pole", new Vector3(0, .1f, 0), new Vector3(.11f, 1.49f, .02f), .039f, Ink.Oak, Family.Wood);
                    Tube(p, "Mop rubber grip", new Vector3(.089f, 1.24f, .017f), new Vector3(.11f, 1.52f, .02f), .06f, Ink.Teal, Family.Rubber);
                    break;
                case "linen":
                case "towel":
                case "dirtytowel":
                case "dirtylinen":
                    ...
                    {
                        GameObject fold = Rounded(p, "Folded textile", new Vector3(j % 2 * .012f, .055f + j * .081f, 0), new Vector3(towel ? .43f : .65f, .104f, towel ? .33f : .43f), dirty ? Ink.Dirty : Ink.White, Family.Cloth);
                        if (dirty) fold.transform.localRotation = Quaternion.Euler(0, j * 9 - 8, j * 3);
                        Box(p, "Woven edge", new Vector3(0, .06f + j * .081f, towel ? -.17f : -.22f), new Vector3(towel ? .38f : .6f, .02f, .011f), dirty ? Ink.Wood : Ink.Teal, Family.Cloth);
                    }
                    if (dirty) Ball(p, "Laundry stain", new Vector3(.12f, towel ? .2f : .28f, -.03f), new Vector3(.2f, .02f, .13f), Ink.Wood, Family.Cloth);
                    else Box(p, "Laundry paper band", new Vector3(0, layers * .081f + .036f, 0), new Vector3(.1f, .012f, towel ? .32f : .44f), Ink.Paper, Family.Paper);
                    break;
                case "trashbag":
                    Ball(p, "Lopsided rubbish bag", new Vector3(0, .25f, 0), new Vector3(.56f, .5f, .5f), Ink.Dark, Family.Rubber);
                    Ball(p, "Bag bulge", new Vector3(.16f, .2f, -.08f), new Vector3(.31f, .31f, .34f), Ink.Dark, Family.Rubber);
                    Shape(p, "Gathered bag neck", cone, new Vector3(0, .51f, 0), new Vector3(.2f, .16f, .2f), Ink.Dark, Family.Rubber);
                    Tube(p, "Bag tie", new Vector3(-.12f, .49f, 0), new Vector3(.12f, .52f, 0), .034f, Ink.Gold, Family.Rubber);
                    break;
                case "cart":
                    Rounded(p, "Luggage cart platform", new Vector3(0, .22f, 0), new Vector3(1.04f, .13f, .68f), Ink.Wood, Family.Wood);
                    Box(p, "Cart carpet", new Vector3(0, .295f, 0), new Vector3(.94f, .025f, .59f), Ink.Burgundy, Family.Carpet);
                    for (int x = -1; x <= 1; x += 2)
                        for (int z = -1; z <= 1; z += 2)
                        {
                            GameObject wheel = Cylinder(p, "Cart wheel", new Vector3(x * .42f, .1f, z * .26f), new Vector3(.17f, .08f, .17f), Ink.Dark, Family.Rubber);
                            wheel.transform.localRotation = Quaternion.Euler(0, 0, 90);
                            Tube(p, "Cart brass upright", new Vector3(x * .47f, .29f, z * .28f), new Vector3(x * .47f, 1.23f, z * .28f), .045f, Ink.Brass, Family.Metal);
                        }
                    for (int z = -1; z <= 1; z += 2)
                    {
                        Tube(p, "Cart upper arch", new Vector3(-.47f, 1.23f, z * .28f), new Vector3(-.28f, 1.43f, z * .28f), .047f, Ink.Brass, Family.Metal);
                        Tube(p, "Cart upper arch", new Vector3(.47f, 1.23f, z * .28f), new Vector3(.28f, 1.43f, z * .28f), .047f, Ink.Brass, Family.Metal);
                        Tube(p, "Cart top bar", new Vector3(-.28f, 1.43f, z * .28f), new Vector3(.28f, 1.43f, z * .28f), .047f, Ink.Brass, Family.Metal);
                    }
                    Handle(p, new Vector3(0, .91f, -.35f), .56f, .13f, Ink.Brass, Family.Metal);
                    break;
                default: // "bag"; a recognizable hard-shell, wheel-mounted suitcase.
                    Rounded(p, "Suitcase shell", new Vector3(0, .37f, 0), new Vector3(.55f, .63f, .3f), Ink.Teal, Family.Rubber);
                    Rounded(p, "Suitcase raised front", new Vector3(0, .38f, -.16f), new Vector3(.45f, .47f, .06f), Ink.Blue, Family.Rubber);
                    for (int j = -1; j <= 1; ++j)
                        Box(p, "Suitcase rib", new Vector3(j * .13f, .38f, -.2f), new Vector3(.018f, .36f, .02f), Ink.Teal, Family.Rubber);
                    for (int j = -1; j <= 1; j += 2)
                    {
                        Box(p, "Leather luggage strap", new Vector3(j * .19f, .37f, -.204f), new Vector3(.041f, .55f, .014f), Ink.Brass, Family.Rubber);
                        Ball(p, "Suitcase wheel", new Vector3(j * .2f, .055f, .03f), new Vector3(.09f, .11f, .09f), Ink.Dark, Family.Rubber);
                    }
                    Handle(p, new Vector3(0, .7f, 0), .25f, .13f, Ink.Dark, Family.Rubber);
                    Rounded(p, "Luggage tag", new Vector3(.13f, .6f, -.23f), new Vector3(.16f, .19f, .027f), Ink.Paper, Family.Paper).transform.localRotation = Quaternion.Euler(0, 0, -13);
        ...
        public static GameObject MakePerson(bool employee, int seed)
        {
            ...
            Rounded(rig.body, "Tailored torso", new Vector3(0, 1.035f, 0), new Vector3(.49f, .59f, .29f), outfit, Family.Cloth);
            Rounded(rig.body, "Trousers hips", new Vector3(0, .723f, 0), new Vector3(.37f, .24f, .27f), Ink.Dark, Family.Cloth);
            for (int side = -1; side <= 1; side += 2)
            {
                Transform leg = Group(rig.body, side < 0 ? "Left leg pivot" : "Right leg pivot", new Vector3(side * .115f, .71f, 0));
                Rounded(leg, "Trouser leg", new Vector3(0, -.265f, 0), new Vector3(.145f, .5f, .19f), Ink.Dark, Family.Cloth);
                Rounded(leg, "Oversized shoe", new Vector3(0, -.615f, .065f), new Vector3(.185f, .16f, .33f), Ink.Wood, Family.Rubber);
                ...
                Rounded(arm, "Jacket sleeve", new Vector3(side * .025f, -.21f, 0), new Vector3(.15f, .43f, .18f), outfit, Family.Cloth);
                Rounded(arm, "Shirt cuff", new Vector3(side * .025f, -.411f, 0), new Vector3(.153f, .063f, .183f), Ink.White, Family.Cloth);
                Ball(arm, "Mitten hand", new Vector3(side * .025f, -.48f, .015f), new Vector3(.155f, .17f, .15f), skin, Family.Skin);
                if (side < 0) rig.leftArm = arm; else rig.rightArm = arm;
            }
            rig.head = Group(rig.body, "Expressive head", new Vector3(0, 1.48f, 0));
            Cylinder(rig.head, "Neck", new Vector3(0, -.095f, 0), new Vector3(.16f, .22f, .16f), skin, Family.Skin);
            Ball(rig.head, "Faceted head", new Vector3(0, .1f, .025f), new Vector3(.41f, .47f, .36f), skin, Family.Skin);
            Ball(rig.head, "Comical nose", new Vector3(0, .063f, .206f), new Vector3(.1f, .13f, .14f), skin, Family.Skin);
            for (int side = -1; side <= 1; side += 2)
            {
                Ball(rig.head, "Ear", new Vector3(side * .208f, .071f, .019f), new Vector3(.07f, .13f, .08f), skin, Family.Skin);
                Ball(rig.head, "Eye white", new Vector3(side * .089f, .155f, .18f), new Vector3(.089f, .082f, .033f), Ink.White, Family.Skin);
                Ball(rig.head, "Dark pupil", new Vector3(side * .08f, .153f, .202f), new Vector3(.037f, .047f, .015f), Ink.Dark, Family.Skin);
                Transform brow = Rounded(rig.head, "Raised eyebrow", new Vector3(side * .089f, .224f, .179f), new Vector3(.104f, .028f, .04f), Ink.Hair, Family.Cloth).transform;
                brow.localRotation = Quaternion.Euler(0, 0, side * (variation % 2 == 0 ? -8 : 11));
                if (side < 0) rig.leftBrow = brow; else rig.rightBrow = brow;
            }
            rig.mouth = Rounded(rig.head, "Wry smile", new Vector3(0, -.028f, .174f), new Vector3(.107f, .022f, .025f), Ink.Hair, Family.Skin).transform;
            Ball(rig.head, "Hair cap", new Vector3(0, .286f, -.014f), new Vector3(.407f, .18f, .344f), Ink.Hair, Family.Cloth);
            Rounded(rig.head, "Unruly fringe", new Vector3(.065f, .249f, .145f), new Vector3(.21f, .1f, .09f), Ink.Hair, Family.Cloth).transform.localRotation = Quaternion.Euler(0, 0, 13);
            Box(rig.body, "Shirt collar", new Vector3(0, 1.249f, .159f), new Vector3(.22f, .105f, .028f), Ink.White, Family.Cloth);
            if (employee)
            {
                for (int j = 0; j < 3; ++j)
                    Ball(rig.body, "Uniform brass button", new Vector3(0, 1.13f - j * .13f, .165f), Vector3.one * .036f, Ink.Brass, Family.Metal);
                Box(rig.body, "Employee badge", new Vector3(-.139f, 1.126f, .162f), new Vector3(.11f, .06f, .027f), Ink.Brass, Family.Metal);
                Cylinder(rig.head, "Bellhop pillbox hat", new Vector3(0, .411f, 0), new Vector3(.36f, .18f, .32f), Ink.Burgundy, Family.Cloth);
                Cylinder(rig.head, "Hat gold piping", new Vector3(0, .477f, 0), new Vector3(.366f, .023f, .326f), Ink.Brass, Family.Cloth);
                Ball(rig.head, "Hat badge", new Vector3(0, .407f, .164f), new Vector3(.07f, .085f, .022f), Ink.Brass, Family.Metal);
            }
            else if (variation == 0)
            {
                Box(rig.body, "Business tie", new Vector3(0, 1.103f, .174f), new Vector3(.067f, .27f, .023f), Ink.Burgundy, Family.Cloth);
                for (int side = -1; side <= 1; side += 2)
                    Rounded(rig.head, "Glasses rim", new Vector3(side * .09f, .152f, .225f), new Vector3(.135f, .1f, .02f), Ink.Brass, Family.Metal);
            }
            else if (variation == 1)
            {
                Box(rig.body, "Tourist camera strap", new Vector3(0, 1.12f, .168f), new Vector3(.035f, .28f, .027f), Ink.Dark, Family.Cloth);
                Rounded(rig.body, "Tourist pocket camera", new Vector3(0, 1.01f, .218f), new Vector3(.22f, .14f, .1f), Ink.Dark, Family.Metal);
                Ball(rig.body, "Camera lens", new Vector3(0, 1.01f, .277f), new Vector3(.095f, .095f, .04f), Ink.Metal, Family.Ceramic);
            }
            else if (variation == 2)
            {
                Cylinder(rig.head, "Sun hat brim", new Vector3(0, .348f, 0), new Vector3(.6f, .04f, .54f), Ink.Linen, Family.Cloth);
                Shape(rig.head, "Sun hat crown", cone, new Vector3(0, .433f, 0), new Vector3(.38f, .19f, .33f), Ink.Oak, Family.Cloth);
            }
            else
                Rounded(rig.body, "Bright scarf", new Vector3(.08f, 1.235f, .2f), new Vector3(.1f, .3f, .1f), Ink.Gold, Family.Cloth);
        ...
        PersonVisual CreatePerson(bool employee, int seed, string id, string label)
        {
            ...
            Rounded(sign, "Name card backing", Vector3.zero, new Vector3(.94f, .3f, .027f), Ink.Paper, Family.Paper);
        ...
        void BuildLighting()
        {
            ...
            sun.type = LightType.Directional; sun.color = new Color(.72f, .81f, .89f);
            sun.intensity = .28f; sun.shadows = LightShadows.Soft; sun.shadowStrength = .72f;
        ...
        static Light PointLight(Transform parent, Vector3 position, float range, float intensity, bool shadows)
        {
            ...
            light.type = LightType.Point; light.range = range; light.intensity = intensity * .65f;
        ...
        {
            Box(p, "Cream plaster wall", basePosition + Vector3.up * (size.y * .5f), size, Ink.Plaster, Family.Plaster, true);
            bool alongX = size.x > size.z;
            Vector3 lower = new Vector3(size.x + (alongX ? 0 : .028f), .98f, size.z + (alongX ? .028f : 0));
            Box(p, "Wooden wainscot", basePosition + Vector3.up * .49f, lower, Ink.Wood, Family.Wood);
            Box(p, "Wainscot chair rail", basePosition + Vector3.up * 1.015f,
                new Vector3(lower.x + (alongX ? 0 : .035f), .085f, lower.z + (alongX ? .035f : 0)), Ink.Oak, Family.Wood);
            Box(p, "Cream crown moulding", basePosition + Vector3.up * 3.43f,
                new Vector3(lower.x + (alongX ? 0 : .06f), .16f, lower.z + (alongX ? .06f : 0)), Ink.Cream, Family.Wood);
            Box(p, "Dark skirting", basePosition + Vector3.up * .07f,
                new Vector3(lower.x + (alongX ? 0 : .02f), .14f, lower.z + (alongX ? .02f : 0)), Ink.Dark, Family.Wood);
            ...
            for (float t = -length * .5f + .38f; t < length * .5f - .1f; t += .72f)
            {
                ...
                Box(p, "Wainscot batten", at, new Vector3(alongX ? .035f : lower.x + .017f, .88f, alongX ? lower.z + .017f : .035f), Ink.Oak, Family.Wood);
            }
            // Large authored repairs remain distinct from the surface texture's decorative wear.
            if (length > 4)
            {
                Vector3 at = basePosition + new Vector3(alongX ? length * .31f : 0, 1.26f, alongX ? 0 : length * .31f);
                Box(p, "Old plaster patch", at, new Vector3(alongX ? .47f : size.x + .009f, .16f, alongX ? size.z + .009f : .47f), Ink.Cream, Family.Plaster);
            }
        }

        static void Rug(Transform p, Vector3 position, Vector2 size)
        {
            Box(p, "Burgundy rug", position, new Vector3(size.x, .025f, size.y), Ink.Burgundy, Family.Carpet);
            for (int s = -1; s <= 1; s += 2)
            {
                Box(p, "Rug side piping", position + new Vector3(s * (size.x * .5f - .085f), .015f, 0), new Vector3(.035f, .007f, size.y - .15f), Ink.Brass, Family.Carpet);
                Box(p, "Rug end piping", position + new Vector3(0, .015f, s * (size.y * .5f - .085f)), new Vector3(size.x - .15f, .007f, .035f), Ink.Brass, Family.Carpet);
            }
        }
        ...
        static void Sofa(Transform p, Vector3 position, float yaw)
        {
            ...
            Rounded(sofa, "Sofa frame", new Vector3(0, .32f, 0), new Vector3(2.36f, .4f, .81f), Ink.Wood, Family.Wood, true);
            Rounded(sofa, "Sofa back", new Vector3(0, .76f, .31f), new Vector3(2.29f, .81f, .23f), Ink.Burgundy, Family.Cloth, true);
            for (int j = -1; j <= 1; ++j)
            {
                Rounded(sofa, "Seat cushion", new Vector3(j * .66f, .54f, -.06f), new Vector3(.63f, .22f, .7f), Ink.Burgundy, Family.Cloth);
                Ball(sofa, "Back button", new Vector3(j * .66f, .82f, .179f), new Vector3(.05f, .05f, .018f), Ink.Brass, Family.Cloth);
            }
            for (int j = -1; j <= 1; j += 2)
            {
                Rounded(sofa, "Rolled armrest", new Vector3(j * 1.08f, .64f, 0), new Vector3(.28f, .39f, .91f), Ink.Burgundy, Family.Cloth, true);
                Rounded(sofa, "Splayed sofa foot", new Vector3(j * .9f, .11f, 0), new Vector3(.16f, .22f, .55f), Ink.Wood, Family.Wood);
            }
            Rounded(sofa, "Faded throw pillow", new Vector3(.61f, .82f, .02f), new Vector3(.44f, .44f, .16f), Ink.Gold, Family.Cloth).transform.localRotation = Quaternion.Euler(10, 0, 17);
        }

        static Transform Lamp(Transform p, Vector3 position, float width)
        {
            Transform lamp = Group(p, "Pleated warm lampshade", position);
            Shape(lamp, "Tapered lampshade", cone, Vector3.zero, new Vector3(width, width * .62f, width), Ink.Linen, Family.Cloth);
            Cylinder(lamp, "Shade lower piping", new Vector3(0, -width * .31f, 0), new Vector3(width * 1.01f, .025f, width * 1.01f), Ink.Burgundy, Family.Cloth);
            Ball(lamp, "Warm bulb", new Vector3(0, -width * .2f, 0), Vector3.one * width * .22f, Ink.Glow, Family.Ceramic);
            if (position.y < 2)
            {
                Tube(lamp, "Bedside lamp stem", new Vector3(0, -width * .32f, 0), new Vector3(0, -.45f, 0), .035f, Ink.Brass, Family.Metal);
                Cylinder(lamp, "Lamp foot", new Vector3(0, -.48f, 0), new Vector3(.29f, .05f, .29f), Ink.Brass, Family.Metal);
            }
            else Tube(lamp, "Pendant cord", new Vector3(0, width * .3f, 0), new Vector3(0, 3.55f - position.y, 0), .024f, Ink.Dark, Family.Rubber);
            return lamp;
        }

        static void Bell(Transform p, Vector3 position)
        {
            Cylinder(p, "Desk bell base", position, new Vector3(.25f, .045f, .25f), Ink.Dark, Family.Metal);
            Ball(p, "Desk bell dome", position + Vector3.up * .05f, new Vector3(.22f, .12f, .22f), Ink.Brass, Family.Metal);
            Cylinder(p, "Bell button", position + Vector3.up * .14f, new Vector3(.07f, .05f, .07f), Ink.Brass, Family.Metal);
        }

        static void Cup(Transform p, Vector3 position)
        {
            Cylinder(p, "Coffee cup", position + Vector3.up * .075f, new Vector3(.12f, .15f, .12f), Ink.Ceramic, Family.Ceramic);
            Cylinder(p, "Cold coffee", position + Vector3.up * .15f, new Vector3(.09f, .007f, .09f), Ink.Wood, Family.Water);
            Handle(p, position + new Vector3(.085f, .05f, 0), .05f, .07f, Ink.Ceramic, Family.Ceramic);
        }

        static void Handle(Transform p, Vector3 at, float width, float height, Ink ink, Family family)
        {
            Tube(p, "Handle left", at + Vector3.left * width * .5f, at + new Vector3(-width * .5f, height, 0), .034f, ink, family);
            Tube(p, "Handle right", at + Vector3.right * width * .5f, at + new Vector3(width * .5f, height, 0), .034f, ink, family);
            Tube(p, "Handle top", at + new Vector3(-width * .5f, height, 0), at + new Vector3(width * .5f, height, 0), .043f, ink, family);
        }

        static void Plant(Transform p, Vector3 at, float height)
        {
            Shape(p, "Terracotta plant pot", cone, at + Vector3.up * .23f, new Vector3(.5f, .46f, .5f), Ink.Red, Family.Ceramic).transform.localRotation = Quaternion.Euler(180, 0, 0);
            Cylinder(p, "Plant soil", at + Vector3.up * .46f, new Vector3(.46f, .025f, .46f), Ink.Wood, Family.Plaster);
            for (int j = 0; j < 7; ++j)
            {
                float a = j * 2.39996f;
                Vector3 tip = at + new Vector3(Mathf.Sin(a) * .33f, height * (.72f + (j % 3) * .13f), Mathf.Cos(a) * .33f);
                Tube(p, "Plant stem", at + Vector3.up * .4f, tip, .02f, Ink.Green, Family.Wood);
                GameObject leaf = Ball(p, "Broad stylized leaf", tip, new Vector3(.2f, .43f, .075f), j % 2 == 0 ? Ink.Leaf : Ink.Green, Family.Skin);
        ...
        static void Frame(Transform p, Vector3 at, float yaw, string caption, float width, float height)
        {
            ...
            Rounded(frame, "Frame", Vector3.zero, new Vector3(width, height, .065f), Ink.Wood, Family.Wood);
            Box(frame, "Print paper", new Vector3(0, 0, -.042f), new Vector3(width - .12f, height - .12f, .014f), Ink.Linen, Family.Paper);
            Text(frame, caption, new Vector3(0, 0, -.052f), width > 1 ? .094f : .063f, Ink.Burgundy);
            Box(frame, "Print top rule", new Vector3(0, height * .32f, -.054f), new Vector3(width * .55f, .016f, .009f), Ink.Brass, Family.Paper);
        }

        static void Clock(Transform p, Vector3 at)
        {
            GameObject outer = Cylinder(p, "Reception clock rim", at, new Vector3(.62f, .055f, .62f), Ink.Brass, Family.Metal);
            outer.transform.localRotation = Quaternion.Euler(90, 0, 0);
            GameObject face = Cylinder(p, "Clock enamel dial", at + Vector3.back * .04f, new Vector3(.53f, .02f, .53f), Ink.Cream, Family.Ceramic);
            face.transform.localRotation = Quaternion.Euler(90, 0, 0);
            Tube(p, "Clock minute hand", at + Vector3.back * .064f, at + new Vector3(.12f, .17f, -.064f), .019f, Ink.Dark, Family.Metal);
            Tube(p, "Clock hour hand", at + Vector3.back * .067f, at + new Vector3(-.12f, .01f, -.067f), .024f, Ink.Dark, Family.Metal);
            for (int j = 0; j < 12; ++j)
            {
                float a = j * Mathf.PI / 6;
                Ball(p, "Clock hour mark", at + new Vector3(Mathf.Sin(a) * .23f, Mathf.Cos(a) * .23f, -.059f), new Vector3(.025f, .025f, .012f), Ink.Wood, Family.Metal);
            }
        }
        ...
        static void Toilet(Transform p, Vector3 at)
        {
            ...
            Ball(toilet, "Toilet pedestal", new Vector3(0, .22f, .03f), new Vector3(.36f, .43f, .47f), Ink.Ceramic, Family.Ceramic);
            Ball(toilet, "Toilet bowl", new Vector3(0, .4f, .13f), new Vector3(.48f, .22f, .65f), Ink.Ceramic, Family.Ceramic);
            Ball(toilet, "Toilet seat", new Vector3(0, .51f, .16f), new Vector3(.49f, .07f, .61f), Ink.White, Family.Ceramic);
            Ball(toilet, "Toilet inner bowl", new Vector3(0, .547f, .16f), new Vector3(.31f, .015f, .41f), Ink.Metal, Family.Ceramic);
            Rounded(toilet, "Cistern", new Vector3(0, .73f, -.21f), new Vector3(.5f, .58f, .22f), Ink.Ceramic, Family.Ceramic);
            Box(toilet, "Cistern lid", new Vector3(0, 1.034f, -.21f), new Vector3(.55f, .05f, .26f), Ink.White, Family.Ceramic);
            Ball(toilet, "Flush button", new Vector3(.16f, .83f, -.083f), Vector3.one * .055f, Ink.Brass, Family.Metal);
        ...
        static void RoomWindow(Transform p, Vector3 at, float sign)
        {
            ...
            Box(window, "Window casing", Vector3.zero, new Vector3(1.96f, 1.73f, .14f), Ink.Wood, Family.Wood);
            Box(window, "Powder-blue daylight", new Vector3(0, 0, -.081f), new Vector3(1.78f, 1.55f, .03f), Ink.Blue, Family.Paper);
            Ball(window, "Distant warm sun", new Vector3(.44f, .37f, -.106f), new Vector3(.29f, .29f, .021f), Ink.Glow, Family.Paper);
            for (int j = 0; j < 4; ++j)
                Ball(window, "Distant stylized hill", new Vector3(-.69f + j * .48f, -.53f, -.119f), new Vector3(.77f, .5f + (j % 2) * .21f, .025f), j % 2 == 0 ? Ink.Teal : Ink.Green, Family.Paper);
            Box(window, "Window mullion", new Vector3(0, 0, -.144f), new Vector3(.065f, 1.63f, .05f), Ink.Cream, Family.Wood);
            Box(window, "Window crossbar", new Vector3(0, .1f, -.147f), new Vector3(1.83f, .07f, .05f), Ink.Cream, Family.Wood);
            Box(window, "Window sill", new Vector3(0, -.87f, -.075f), new Vector3(2.03f, .13f, .29f), Ink.Cream, Family.Wood);
            for (int side = -1; side <= 1; side += 2)
                for (int j = 0; j < 3; ++j)
                    Cylinder(window, "Burgundy curtain pleat", new Vector3(side * (.77f + j * .11f), -.035f, -.22f), new Vector3(.16f, 1.87f, .12f), Ink.Burgundy, Family.Cloth);
            Tube(window, "Curtain rod", new Vector3(-1.15f, .98f, -.19f), new Vector3(1.15f, .98f, -.19f), .04f, Ink.Brass, Family.Metal);
        }
        ...
        static GameObject Shape(Transform parent, string name, Mesh mesh, Vector3 position, Vector3 size, Ink ink, Family family, bool solid = false)
        {
            ...
            renderer.sharedMaterial = HotelTextureLibrary.GetMaterial(family, ink, HotelMappingMode.UV0);
        ...
        static GameObject Box(Transform p, string name, Vector3 at, Vector3 size, Ink ink, Family family, bool solid = false) => Shape(p, name, cube, at, size, ink, family, solid);
        static GameObject Rounded(Transform p, string name, Vector3 at, Vector3 size, Ink ink, Family family, bool solid = false) => Shape(p, name, bevel, at, size, ink, family, solid);
        static GameObject Ball(Transform p, string name, Vector3 at, Vector3 size, Ink ink, Family family) => Shape(p, name, sphere, at, size, ink, family);
        static GameObject Cylinder(Transform p, string name, Vector3 at, Vector3 size, Ink ink, Family family, bool solid = false) => Shape(p, name, cylinder, at, size, ink, family, solid);

        static void Tube(Transform p, string name, Vector3 a, Vector3 b, float diameter, Ink ink, Family family)
        {
            Vector3 difference = b - a;
            GameObject tube = Cylinder(p, name, (a + b) * .5f, new Vector3(diameter, difference.magnitude, diameter), ink, family);
        ...
        static void Tint(Renderer renderer, Ink ink) => renderer.sharedMaterial = HotelTextureLibrary.WithInk(renderer.sharedMaterial, ink);
        static Color ColorOf(Ink ink) => HotelTextureLibrary.ColorOf(ink);

        // Meshes use independent face vertices for deliberate flat shading. Winding is verified
        // against an outward direction. UVs do not change the vertices, winding or collision bounds.
        ...
        static Mesh Lathe(Vector2[] profile, int segments, string name)
        {
            ...
            var distance = new float[profile.Length];
            for (int j = 1; j < profile.Length; ++j)
                distance[j] = distance[j - 1] + Vector2.Distance(profile[j - 1], profile[j]);
            float length = Mathf.Max(distance[distance.Length - 1], .0001f);
            for (int j = 0; j < profile.Length - 1; ++j)
                for (int i = 0; i < segments; ++i)
                {
                    ...
                    // Duplicate face vertices already exist, so the angular seam can use both 0/1
                    // without stretching across the cylinder. Flat caps use a planar disc instead.
                    float u0 = i / (float)segments, u1 = (i + 1) / (float)segments;
                    float v0 = distance[j] / length, v1 = distance[j + 1] / length;
                    bool cap = Mathf.Abs(tangent.y) < .0001f;
                    Vector2 puv = cap ? new Vector2(p.x + .5f, p.z + .5f) : new Vector2(u0, v0);
                    Vector2 quv = cap ? new Vector2(q.x + .5f, q.z + .5f) : new Vector2(u1, v0);
                    Vector2 ruv = cap ? new Vector2(r.x + .5f, r.z + .5f) : new Vector2(u1, v1);
                    Vector2 suv = cap ? new Vector2(s.x + .5f, s.z + .5f) : new Vector2(u0, v1);
                    if (profile[j].x < .0001f)
                    {
                        Face(vertices, triangles, normal, p, r, s);
                        if (!cap) puv.x = (u0 + u1) * .5f;
                        uvs.Add(puv); uvs.Add(ruv); uvs.Add(suv);
                    }
                    else if (profile[j + 1].x < .0001f)
                    {
                        Face(vertices, triangles, normal, p, q, r);
                        if (!cap) ruv.x = (u0 + u1) * .5f;
                        uvs.Add(puv); uvs.Add(quv); uvs.Add(ruv);
                    }
                    else
                    {
                        Face(vertices, triangles, normal, p, q, r, s);
                        uvs.Add(puv); uvs.Add(quv); uvs.Add(ruv); uvs.Add(suv);
                    }
                }
            return MeshFrom(vertices, triangles, name, uvs);
        }
        ...
        static Mesh MeshFrom(List<Vector3> vertices, List<int> triangles, string name, List<Vector2> uvs = null)
        {
            Mesh mesh = new Mesh { name = name }; mesh.SetVertices(vertices); mesh.SetTriangles(triangles, 0);
            mesh.RecalculateNormals();
            if (uvs == null)
            {
                // Face-local box/chamfer UVs: one shared mesh, no transform-dependent coordinates.
                ...
                for (int i = 0; i < vertices.Count; ++i)
                {
                    ...
                    Vector2 uv = a.x >= a.y && a.x >= a.z ? new Vector2(-Mathf.Sign(n.x) * p.z, p.y) :
                        a.y >= a.z ? new Vector2(p.x, -Mathf.Sign(n.y) * p.z) : new Vector2(Mathf.Sign(n.z) * p.x, p.y);
                    uvs.Add(uv + Vector2.one * .5f);
                }
            }
            mesh.SetUVs(0, uvs);
